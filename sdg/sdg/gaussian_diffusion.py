...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        betas = np.array(betas, dtype=np.float64) # 步长是1000
        self.betas = betas
        assert len(betas.shape) == 1, "betas must be 1-D"
        assert (betas > 0).all() and (betas <= 1).all()

        self.num_timesteps = int(betas.shape[0]) # 采样步长 1000
    ...
    def condition_mean(self, cond_fn, p_mean_var, x, t, model_kwargs=None):
        ...
        """
        # sgd 梯度
        # text_loss 关于 x 的梯度，与x的维度一致
        gradient = cond_fn(x, self._scale_timesteps(t), **model_kwargs)
        # 加入条件后就等于是均值进行了移动，具体移动公式这样计算。
        # 这边没加▽xt-1 logP(t|xt-1)
        # 就是在xt-1的生成图像下，能得出该图像是符合y描述的概率，最后再对x进行求导
        # 这一步就和论文对上了，牛逼，而Unet那一块啥也没用到
        new_mean = (
    ...
    def p_sample(
        ...
        if cond_fn is not None:
            # 找到了，关于文本是如何加进去的，就是计算一个偏移均值的正态分布
            # 算出新的均值，根据论文3.2节的算法
            out["mean"] = self.condition_mean(
                cond_fn, out, x, t, model_kwargs=model_kwargs
            )
        # 采样的计算公式（而且均值和方差是针对像素的）
    ...
    def p_sample_loop_progressive(
        ...
        indices = list(range(self.num_timesteps))[::-1] # [::-1] 将列表翻转了一下
        ...
        for i in indices:
            ...
            with th.no_grad():
                out = self.p_sample(
                    ...
                    cond_fn=cond_fn, # 有
                    model_kwargs=model_kwargs, # guidance condition
                )
                yield out
                img = out["sample"]
